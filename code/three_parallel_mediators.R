...
if(any(abs(corMat) > 1)) {
  ...
  a3 <- as.numeric(input$STa3)
  b1 <- as.numeric(input$STb1)
  b2 <- as.numeric(input$STb2)
  b3 <- as.numeric(input$STb3)
  cor32 <- as.numeric(input$cor32)
  cor42 <- as.numeric(input$cor42)
  cor43 <- as.numeric(input$cor43)
  ...
  if(abs(a1)> .999 | abs(a2)> .999 | abs(a3) > .999 |
     ...
     abs(cprime)> .999 | abs(cor32) > .999 | abs(cor42) > .999 |
     abs(cor43) > .999
     ) {
...
if (input$obj == "choose_n") {
  ...
  withProgress(message = 'Running Replications', value = 0, {
    ...
    names(pow) <- c("N", "a1b1", "a2b2", "a3b3", "difference12", "difference13", "difference23")
    ...
    df <- data.frame("Parameter" = rep(c("a1b1", "a2b2", "a3b3", "difference12", "difference13", "difference23"),
                                       each = length(Nused)),
                     do.call("rbind", out))
    # Check:
    if (all(df$Power < TarPow)) {
      showNotification("Power for all sample sizes and indirect effects is less than the target power value.
           Consider choosing a larger upper sample size", type = "warning")
    }
